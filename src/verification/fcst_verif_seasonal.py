...
class Forecast:
    ...
    def check_models(self, base, year_fcst, month_fcst, var):
        ...
        fcst_dir = Path(self.path_fcst)

        if base == "copernicus":
            models = ["cmccs35", "ecccs4", "ecccs5",  "ecmwfs5",  "jmas3",  "mfs9", "ncep", "dwds22", "ukmos6v604", "boms2", "bam12"]
        elif base == "nmme":
        ...
        for mdl in models:
            file_path = fcst_dir / base / mdl / str(year_fcst) / f"{var}_monthly_{mdl}_fcst_interp_{year_fcst}{month}01.nc"
    ...
    def read_fcst_file(self, base, year_fcst, month_fcst, model, var):
        ...
        fcst_dir = f"{self.path_fcst}/{base}/{model}/{year_fcst}/"
        file_name_fcst = os.path.join(fcst_dir, f"{var}_monthly_{model}_fcst_interp_{year_fcst}{month}01.nc")
    ...
    def forecast_multimodel(self, base, year_fcst, month_fcst, var):    
        ...
        if base == "copernicus":
            models_all = ["cmccs35", "ecccs4", "ecccs5",  "ecmwfs5",  "jmas3",  "mfs9", "ncep", "dwds22", "ukmos6v604", "boms2", "bam12"]
        models_available = self.check_models(base, year_fcst, month_fcst, var)
        print(f"Modelos disponíveis (forecast): {models_available}")
        #models_available = ["gefsv12", "cfsv2", "esrl"]  # se for fixo, pode deixar assim
        #print(f"Modelos disponíveis:{models_available}")
        ...
        for model in models_all: 
            if model in models_available:
                ...
                fcst_arrays[model] = self.dict_to_array(base, fcst_dict, ["mnth00","mnth01","mnth02","mnth03","mnth04","seas00","seas01","seas02"], var)
            else:
                fcst_arrays[model] = np.full(shape, np.nan)
        stacked = np.stack([fcst_arrays[m] for m in models_all], axis=0)
...
# month_fcst = 5
# var = "prec"
# fcst = Forecast(path_fcst)

# realtime_fcst = fcst.forecast_multimodel(base, year_fcst, month_fcst, var)
# #realtime_fcst_arr = np.stack([realtime_fcst[k] for k in realtime_fcst.keys()], axis=0)
# print(realtime_fcst.shape)
