...
*************MODEL*****************
model_dir=subwrd(args,3)
model_title=subwrd(args,4)
say model_dir
say model_title
***********************************
...
*************VARIÁVEL**************
namevar=subwrd(args,7)
say namevar
***********************************

*************CALIBRAÇÃO**************
calibration=subwrd(args,8)
say calibration
***********************************
...
**canesm5 ccsm4 cesm1 cfsv2 gem52nemo geos5v2 spear bam12**
**CanESM5 NCAR_CCSM4 NCAR_CESM1 CFSv2 GEM5.2_NEMO NASA_GEOS5v2 GFDL_SPEAR CPTEC_BAM1.2**
    ...
    while(lt<=ltf) 
        ...
        while(meti<=metf)
            ...
            if (calibration = 'nocalib')
                if (base = 'copernicus')
                    path_in = '/dados/mmclima/multimodelo/seasonal/posproc/copernicus/'version'/verification/nocalib/' % model_dir % '/'
                    out     = '/dados/mmclima/multimodelo/seasonal/figures/copernicus/'version'/verification/nocalib/' % model_dir % '/'
                    '!mkdir -p 'out'' 
                endif
                if (base = 'nmme')
                    path_in = '/dados/mmclima/multimodelo/seasonal/posproc/nmme/'version'/verification/nocalib/' % model_dir % '/'
                    out     = '/dados/mmclima/multimodelo/seasonal/figures/nmme/'version'/verification/nocalib/' % model_dir % '/'
                    '!mkdir -p 'out'' 
                endif
                file = namevar % "_" % met % "_" % plt % lt % "_" % model_dir % "_nocalib_" % anomesdiahora % ".nc"
            endif

            if (calibration = 'regr')
                if (base = 'copernicus')
                    path_in = '/dados/mmclima/multimodelo/seasonal/posproc/copernicus/'version'/verification/regr/' % model_dir % '/'
                    out     = '/dados/mmclima/multimodelo/seasonal/figures/copernicus/'version'/verification/regr/' % model_dir % '/'
                    '!mkdir -p 'out'' 
                endif
                if (base = 'nmme')
                    path_in = '/dados/mmclima/multimodelo/seasonal/posproc/nmme/'version'/verification/regr/' % model_dir % '/'
                    out     = '/dados/mmclima/multimodelo/seasonal/figures/nmme/'version'/verification/regr/' % model_dir % '/'
                    '!mkdir -p 'out'' 
                endif
                file = namevar % "_" % met % "_" % plt % lt % "_" % model_dir % "_calibrated_regr_" % anomesdiahora % ".nc"
            endif

            if (calibration = 'cox')
                if (base = 'copernicus')
                    path_in = '/dados/mmclima/multimodelo/seasonal/posproc/copernicus/'version'/verification/cox/' % model_dir % '/'
                    out     = '/dados/mmclima/multimodelo/seasonal/figures/copernicus/'version'/verification/cox/' % model_dir % '/'
                    '!mkdir -p 'out'' 
                endif
                if (base = 'nmme')
                    path_in = '/dados/mmclima/multimodelo/seasonal/posproc/nmme/'version'/verification/cox/' % model_dir % '/'
                    out     = '/dados/mmclima/multimodelo/seasonal/figures/nmme/'version'/verification/cox/' % model_dir % '/'
                    '!mkdir -p 'out'' 
                endif
                file = namevar % "_" % met % "_" % plt % lt % "_" % model_dir % "_calibrated_cox_" % anomesdiahora % ".nc"
            endif
            ...
            if (base = 'copernicus')
                ...
                if (calibration = 'nocalib') 
                    if (namevar = 'prec') 
                        if (model_dir != 'bam12')
                            l1 = metn % ": " % model_title % " (C3S)  REF: GPCP"
                        endif
                        if (model_dir = 'bam12')
                            l1 = metn % ": " % model_title % "  REF: GPCP"
                        endif                                    
                    endif
                    if (namevar = 't2mt')     
                        if (model_dir != 'bam12')
                            l1 = metn % ": " % model_title % " (C3S)  REF: ERA5"
                        endif
                        if (model_dir = 'bam12')
                            l1 = metn % ": " % model_title % "  REF: ERA5"
                        endif                                         
                    endif                                
                endif
                ...
                if (calibration = 'regr')
                    if (namevar = 'prec') 
                        if (model_dir != 'bam12')
                            l1 = metn % ": " % model_title % " (C3S) CALIBRATED (REGR.) REF: GPCP"
                        endif
                        if (model_dir = 'bam12')
                            l1 = metn % ": " % model_title % " CALIBRATED (REGR.) REF: GPCP"
                        endif                                    
                    endif
                    if (namevar = 't2mt')     
                        if (model_dir != 'bam12')
                            l1 = metn % ": " % model_title % " (C3S) CALIBRATED (REGR.) REF: ERA5"
                        endif
                        if (model_dir = 'bam12')
                            l1 = metn % ": " % model_title % " CALIBRATED (REGR.) REF: ERA5"
                        endif                                          
                    endif                      
                endif
                ...
                if (calibration = 'cox')
                    if (namevar = 'prec')
                        if (model_dir != 'bam12')
                            l1 = metn % ": " % model_title % " (C3S) CALIBRATED (COX) REF: GPCP"
                        endif
                        if (model_dir = 'bam12')
                            l1 = metn % ": " % model_title % " CALIBRATED (COX) REF: GPCP"
                        endif                                 
                    endif
                    if (namevar = 't2mt')
                        if (model_dir != 'bam12')
                            l1 = metn % ": " % model_title % " (C3S) CALIBRATED (COX) REF: ERA5"
                        endif
                        if (model_dir = 'bam12')
                            l1 = metn % ": " % model_title % " CALIBRATED (COX) REF: ERA5"
                        endif                                    
                    endif                      
                endif
            endif
            ...
            if (base = 'nmme')
                ...
                if (calibration = 'nocalib') 
                    if (namevar = 'prec') 
                        if (model_dir != 'bam12')
                            l1 = metn % ": " % model_title % " (NMME)  REF: GPCP"
                        endif
                        if (model_dir = 'bam12')
                            l1 = metn % ": " % model_title % "  REF: GPCP"
                        endif                                    
                    ...
                    if (namevar = 't2mt')     
                        if (model_dir != 'bam12')
                            l1 = metn % ": " % model_title % " (NMME)  REF: ERA5"
                        endif
                        if (model_dir = 'bam12')
                            l1 = metn % ": " % model_title % "  REF: ERA5"
                        endif                                         
                ...
                if (calibration = 'regr')
                    if (namevar = 'prec') 
                        if (model_dir != 'bam12')
                            l1 = metn % ": " % model_title % " (NMME) CALIBRATED (REGR.) REF: GPCP"
                        endif
                        if (model_dir = 'bam12')
                            l1 = metn % ": " % model_title % " CALIBRATED (REGR.) REF: GPCP"
                        endif                                    
                    ...
                    if (namevar = 't2mt')     
                        if (model_dir != 'bam12')
                            l1 = metn % ": " % model_title % " (NMME) CALIBRATED (REGR.) REF: ERA5"
                        endif
                        if (model_dir = 'bam12')
                            l1 = metn % ": " % model_title % " CALIBRATED (REGR.) REF: ERA5"
                        endif                                          
                ...
                if (calibration = 'cox')
                    if (namevar = 'prec')
                        if (model_dir != 'bam12')
                            l1 = metn % ": " % model_title % " (NMME) CALIBRATED (COX) REF: GPCP"
                        endif
                        if (model_dir = 'bam12')
                            l1 = metn % ": " % model_title % " CALIBRATED (COX) REF: GPCP"
                        endif                                 
                    ...
                    if (namevar = 't2mt')
                        if (model_dir != 'bam12')
                            l1 = metn % ": " % model_title % " (NMME) CALIBRATED (COX) REF: ERA5"
                        endif
                        if (model_dir = 'bam12')
                            l1 = metn % ": " % model_title % " CALIBRATED (COX) REF: ERA5"
                        endif                                    
...
****AF****
            ...
            if (base = 'nmme')
                if (calibration = 'nocalib')
                    fileout = "nmme_"  % model_dir % "_nocalib_" % metname % "_" % namevar % "_" % plt % lt % "_" % namemonth % "_" % reg
                endif
                if (calibration = 'regr')
                    fileout = "nmme_"  % model_dir % "_regr_" % metname % "_" % namevar % "_" % plt % lt % "_" % namemonth % "_" % reg
                endif
                if (calibration = 'cox')
                    fileout = "nmme_"  % model_dir % "_cox_" % metname % "_" % namevar % "_" % plt % lt % "_" % namemonth % "_" % reg
                endif        
            endif

            if (base = 'copernicus')
                if (calibration = 'nocalib')
                    fileout = "c3s_"  % model_dir % "_nocalib_" % metname % "_" % namevar % "_" % plt % lt % "_" % namemonth % "_" % reg
                endif
                if (calibration = 'regr')
                    fileout = "c3s_"  % model_dir % "_regr_" % metname % "_" % namevar % "_" % plt % lt % "_" % namemonth % "_" % reg
                endif
                if (calibration = 'cox')
                    fileout = "c3s_"  % model_dir % "_cox_" % metname % "_" % namevar % "_" % plt % lt % "_" % namemonth % "_" % reg
                endif           
            endif
